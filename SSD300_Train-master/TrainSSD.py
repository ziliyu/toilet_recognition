...
class TrainSSD(object):
    ...
    def SD_fn_TrainSSD(self):
        ...
        epochss = range(1, len(train_loss) + 1)
        plt.plot(epochss, train_loss, 'bo', c='b', label='training loss')
        plt.plot(epochss, test_loss, 'b', c='g', label='test loss')
    ...
    def SD_fn_Predict(self, img, min_score=0.6):
        ...
        preds = self.model.predict(inputs, batch_size=5, verbose=0)      # verbose = 1 显示耗时
...
if __name__ == '__main__':
    ...
    my_ssd = TrainSSD(weights_path='./weights/weights_SSD300.hdf5',      # 权值文件，后期可更改为训练好的权值文件，以实继续训练
                      Label_Data_path='my_new_data.pkl',                # 教程5.1中生成的标记数据文件，包含标注信息
                      ImgDir_path='D:\datajihe\wash\\',            # 数据集图片保存位置--文件夹
